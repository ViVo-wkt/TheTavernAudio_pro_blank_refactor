...
        // 2. TWORZENIE INSTANCJI DŹWIĘKU 
    ...
    void HandleChargingState()
    {
        ...
        {
            // TUTAJ JEST KLUCZOWA ZMIANA LOGIKI AUDIO
            if (chargeTimer >= chargeTime)
            {
                // Scenariusz 1: PEŁNE NAŁADOWANIE -> STRZAŁ
                FireSpell("released at end of CHARGING");

                // Odtwarzamy dźwięk wystrzału (Launch) tylko tutaj
                if (isAudioInitialized)
                    audioSystem.SpellRelease();
            }
            else
            {
                // Scenariusz 2: PRZERWANIE ŁADOWANIA -> BRAK STRZAŁU
                CancelSpell();

                // Tylko wyciszamy dźwięk ładowania (bez Launch)
                if (isAudioInitialized)
...
        if (Input.GetMouseButtonDown(1))
        {
            ...
            {
                audioSystem.SpellCancel();
                // Tutaj SpellRelease() byłby błędem, bo zagrałby strzał. Wystarczy SpellCancel.
            }
    ...
    void HandleHoldingState()
    {
        ...
        if (Input.GetMouseButtonUp(0))
        {
            ...
            // --- LOGIKA AUDIO: W stanie Holding zawsze jest full charge -> GRAMY STRZAŁ
            if (isAudioInitialized)
                audioSystem.SpellRelease();
